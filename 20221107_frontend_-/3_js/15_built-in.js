...
/* 
    전역 객체
        ...
        - ES11에서 도입된 globalThis를 통해서 실행환경 상관없이 접근 가능하다.
*/
console.log(globalThis); // globalThis를 통해서 실행환경 관계 없이 전역 객체에 접근이 가능하다.
...
function sum(a, b) {
    num = 10; // var 생략 시 함수 안에서 작성해도 함수 레벨 스코프가 지원안되기 때문에 window.num 프로퍼티로 등록된다. 이렇게 쓰지 말라고 알려줌
    return a + b;
}

for (var i = 0; i < 10; i++) {} // var 키워드는 블록 레벨 스코프를 지원 안하기 때문에 window.i 프로퍼티에 등록된다.
...
// Math : 수학 관련 프로퍼티와 메서드 제공 => 주로 정적 프로퍼티(메서드 포함)를 사용한다.
console.log(Math.PI * 2 * 10);

console.log(Math.sin(Math.PI / 4));

let float = 1.52412321;
let int = parseInt(float); // parseInt() : 실수를 정수로 바꿀 때도 사용한다. 소수저 ㅁ이하의 숫자를 버린다(내림).
...
function getLottonum() {
    ...
    // result 배열의 길이가 6이 되기 전까지만 실행 => 요소가 6개 되는 순간 반복을 멈춘다.
