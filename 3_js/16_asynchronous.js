/* 
    동기(Synchronous)와 비동기(Asynchronous)
        ...
        - 비동기 코드는 순서를 보장하지 않는다. 실행결과를 기다리지 않고 다음 코드를 실행한다.
            ...
            => 주로 서버에서 데이터를 받아오거나 이미지 파일을 로딩하는 것처럼 오래 걸리는 함수들 중에 비동기 함수가
        - JavaScript는 기본적으로 동기적인 프로그래밍언어이다.
...
/* 
    Promise : 비동기 코드를 쉽게 처리할 수 있는 객체.
        ...
        => reject() : 실패했을 때. 에러를 발생시킨다. Promise.prototype.catct의 첫번째 인자로 전달된다.
            => catch() 에러를 받아서 처리할 때 사용하는 함수. 에러처리를 안하면 에러가 발생해서 이후의 코드가 실행되지 않는다.
*/
...
/* 
    async/await : 비동기 처리 함수
        - async 키워드를 함수 앞에 붙이면 Promise를 동기적으로 쉽게 처리할 수 있는 함수로 변경된다.
        - Promise를 반환하는 함수 앞에 await을 붙이면 resolve 혹은 reject 되기 전까지 다음 코드를 실행하지 않는다.
            => Promise의 결과를 반환한다. => await이 없으면 Promise 객체 자체를 반환한다.
            => async 함수 내에서 참조하면 resolve 값을 잘 참조하지만, async 함수의 리턴값은 해당값을 resolve 하는 Promise가 반환된다.
*/

async function renderMovie(title) {
    // Promise를 반환하는 함수 앞에 await을 붙이면 resolve/reject 되기 전까지 다음 코드 실행하지 않는다.
