...
/* 
    클래스(class)
        - 생성자 함수와 동일한 역할을 한다.
            ...
            => 문법적 설탕(Syntax Sugar)
        - constroctor를 통해서 초기화를 진행할 수 있다.
            => 전달해야하는 초기값을 constroctor의 매개 변수로 전달받을 수 있다.
            => 초기화할 값이 없다면 생략할 수 있다.
        - new 없이 호출하면 오류가 발생한다.
        - extends를 통해 클래스를 상속받을 수 있다.
            => 상속하는 클래스를 슈퍼 클래스, 상속받는 클래스를 서브 클래스라고 한다.
        - 서브(하위) 클래스에 추가적으로 초기화할 값이 있다면 먼저 super(상위 클래스의 constructor)를 호출해야 한다.
            => super를 통해 상위 클래스의 초기화값을 전달하고 추가적으로 값을 초기화하면 된다.
*/
class Animal {
    constructor(legs) {
        this.legs = legs;
...
// 문제1. Animal을 상속받는 Cat 클래스 만들어보기
// species(고양이 종류) 속성 추가
// meow 함수(인스턴스 메서드) => 울음 소리 내는 메서드
