...
// 보통 let을 사용하고, var는 옛날 코드를 위해서 사용
let price = 1000;
// let price = 10000;

// const - 상수(Constant)는 변하지 않는 수. => 재할당을 하면 오류가 난다. 변하면 안되는 값을 할당한다.
//  => 상수 선언 시 값을 초기화하지 않으면 오류 발생.
const TAX_RATIO = 10;
// TAX_RATIO = 100;
...
// 스코프 체인(scope chain)
// es6 - 최신 문법의 기준
// ecma스크립트 - ecma-262(JS 표준화)

// 전역 스코프 : 가장 바깥 유효 범위. 어디서든 접근 가능하다. 가장 상위 스코프.
// let num = 10; 이미 var num = 10; Identifier 'num' has already been declared
let value = 777;

// 지역 스코프 : 코드 블록 내의 범위. 해당 블록에서만 유효하다.
//  => 하위 스코프에서 상위 스코프를 참조하는 건 가능하지만, 반대는 안된다.
...
// console.log(str); 바깥에서 선언되지 않았기 때문에 str is not defined
